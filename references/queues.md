...
Define limiters in `AppServiceProvider::boot()` then use `RateLimited` job middleware:
...
{
    RateLimiter::for('process-posts', function (object $job) {
        return Limit::perMinute(10)->by($job->post->id ?? $job->uniqueId());
    });
}
```
...
For per-user rate limiting, define a dynamic limiter:

```php
RateLimiter::for('process-posts-by-user', function (object $job) {
    return Limit::perMinute(10)->by($job->user?->id ?? $job->uniqueId());
});
```
...
class RateLimitedByUser
{
    ...
    {
        $key = 'job-user:' . ($job->user?->id ?? $job->uniqueId());
...
// WithoutOverlapping — prevent same job running concurrently
