...
> **Livewire 3+**: Use `dispatch()` instead of `emit()`. Use `wire:model.live` or `wire:model.blur` instead of `wire:model.defer` (which still works).
...
<form wire:submit.prevent="save">
    {{-- Title: wire:model.blur defers until blur (Livewire 3); .defer still works --}}
    <div>
...
// Listen to event (Livewire 3: use #[On] attribute)
class PostStats extends Component
{
    #[On('postDeleted')]
    public function updateStats(int $postId): void
    {
        // Update statistics
    }
}
...
// Dispatch to parent/child
$this->dispatch('saved')->to('parent-component');
$this->dispatch('refresh')->self();

// Browser events (Alpine/JS listeners)
...
1. **Use wire:model.blur or wire:model.defer** - Batch updates on blur/submit; avoid live on every keystroke for forms
