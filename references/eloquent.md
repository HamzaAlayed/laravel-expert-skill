...
class Post extends Model
{
    // Local scope — return $query for chainability
...
```php
// Preferred: Attribute syntax (Laravel 9+)
use Illuminate\Database\Eloquent\Casts\Attribute;
...
// Alternative: Cast class for complex logic
namespace App\Casts;
...
// When conditional queries (escape LIKE wildcards if $search is user input)
$posts = Post::query()
...
1. **Always eager load relationships** - Avoid N+1 queries (use `with()`)
